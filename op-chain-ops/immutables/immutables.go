...
func l2ImmutableDeployer(backend *backends.SimulatedBackend, opts *bind.TransactOpts, deployment deployer.Constructor) (*types.Transaction, error) {
	...
	case "DeveloperList":
		admin, ok := deployment.Args[0].(common.Address)
		if !ok {
			return nil, fmt.Errorf("invalid type for developerList admin")
		}
		_, tx, _, err = bindings.DeployDeveloperList(opts, backend, admin)
